        ...
        if (!statuses.isEmpty()) {
            ...
            for (Status status : statuses) {
                if (status.getErrorCount() >= errorFailCount) {
                    ...
                    TaxiiServer server = association.getServer();
                    TaxiiCollection collection = association.getCollection();
                    try {
                        ...
                        log.debug("[x] Error updating status(id={},server={},collection={},trace_config={}), errors({}/{}): {}", status.getId(), server, collection, traceConfiguration, status.getErrorCount(), errorFailCount, details);
