...
public class UploadService {
    ...
    @Value("${flare.timeout.taxii-21.upload:5000}")
    private int taxii21UploadTimeout;
    ...
    public String publish(Taxii21Association association, Map<String, UploadedFile> fileMap) {
        ...
        for (Map.Entry<String, UploadedFile> uploadedFile: fileMap.entrySet()) {
            ...
            try {
                restTemplate.setTimeouts(taxii21UploadTimeout);
                response = restTemplate.postEnvelope(association.getServer(), url, envelope);
            } catch (TaxiiErrorResponseException ex) {
                TaxiiError error = ex.getTaxiiError();
                String feedback = String.format("Received HTTP(%s): %s", error.getHttpStatus(), error.getDescription());
                log.error("[x] Error publishing: " + feedback, ex);
