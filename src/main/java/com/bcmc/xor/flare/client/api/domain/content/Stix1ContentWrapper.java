...
public class Stix1ContentWrapper extends AbstractContentWrapper {
    ...
    public Stix1ContentWrapper(Object contentObject, TaxiiAssociation taxiiAssociation) {
        ...
        if (this.contentObject == null) {
            this.setValidationResult(new ValidationResult(ValidationResult.Status.ERROR, Sets.newHashSet("Content unable to parsed")));
        } else {
    ...
    private String hash(String object) {
        try {
            return DatatypeConverter.printHexBinary(MessageDigest.getInstance("MD5").digest(this.contentObject.getBytes())).toUpperCase();
    ...
    private String contentObjectToString(Object contentObject) {
        ...
        if (contentObject instanceof ElementNSImpl) {
            // NOTE: .getAttribute() returns an empty string, not null. id, version, timestamp may be empty strings.
            ...
            this.setContentTimestamp(DateTimeFormatter.ISO_DATE_TIME.parse( ((ElementNSImpl) contentObject).getAttribute("timestamp") , ZonedDateTime :: from ));
            ...
            try {
                ...
                this.setContentBinding(document.getDocumentElement().getAttributes().getNamedItem("version").getNodeValue());
                this.setContentTimestamp(DateTimeFormatter.ISO_DATE_TIME.parse(document.getDocumentElement().getAttributes().getNamedItem("timestamp").getNodeValue(), ZonedDateTime::from));
    ...
    public ValidationResult validate() {
        HashSet<String> errors = Validation.getInstance().validateAndReturnErrors(this.contentObject, this.contentBinding);
        this.getValidationResult().setStatus(errors.isEmpty() ? ValidationResult.Status.VALID : ValidationResult.Status.INVALID);
