...
class CollectionResource {
    ...
    public ResponseEntity<String> purgeCollection(@PathVariable String serverLabel, @PathVariable String collectionId) {
        log.debug("REST Request to purge server '{}' collection '{}'", serverLabel, collectionId);
        Optional<? extends TaxiiServer> server = serverService.findOneByLabel(serverLabel);
        if (server.isPresent()) {
            if (server.get().getCollections() != null) {
                ...
                if (collections.getAllIds().contains(collectionId)) {
                    ...
                    return new ResponseEntity<>("Successfully purged collection '" + collectionId + "'", HttpStatus.OK);
                ...
                log.error("Server has no collection to purge");
                return new ResponseEntity<>("Server has no collection to purge", HttpStatus.OK);
            }
        } else {
            log.error("Server {} not found", serverLabel);
            throw new ServerNotFoundException();
