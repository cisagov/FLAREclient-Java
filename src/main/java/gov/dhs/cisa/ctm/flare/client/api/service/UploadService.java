...
import org.mitre.taxii.messages.xml11.RecordCountType;
...
public class UploadService {
	...
	public UploadService(EventService eventService, TaxiiService taxiiService, StatusService statusService,
			ServerService serverService, CollectionService collectionService, UserService userService) {
	...
	public String publish(Taxii11Association association, Map<String, UploadedFile> fileMap) {
		...
		for (Map.Entry<String, UploadedFile> uploadedFile : fileMap.entrySet()) {
			...
			if (document == null) {
				eventService.createEvent(EventType.PUBLISH_FAILED,
						String.format("Failed to parse XML file %s.", uploadedFile.getKey()), association);
				continue;
			}

			ContentBlock contentBlock = new ContentBlock();
			contentBlock.withContent(
					new AnyMixedContentType(Collections.singletonList(uploadedFile.getValue().getContent())));
			// Establish content binding for this package
			ContentInstanceType contentInstanceType = new ContentInstanceType(null,
					// If package has "version", get it, otherwise, default to 1.1.1 (because we
					// have to validate against some schema, so we'll assume most common 1X data)
					Constants.ContentBindings.contentBindingMap
							.get(document.getDocumentElement().hasAttribute("version")
			...
			contentBlock.withTimestampLabel(document.getDocumentElement().hasAttribute("version")
					? new XMLGregorianCalendarImpl(GregorianCalendar
							.from(ZonedDateTime.parse(document.getDocumentElement().getAttribute("timestamp"))))
					: null);
			// Add to list of content blocks
			contentBlocks.add(contentBlock);
		}

		InboxMessage inboxMessage = (new InboxMessage()).withMessageId(MessageHelper.generateMessageId())
				.withDestinationCollectionNames(association.getCollection().getName()).withContentBlocks(contentBlocks)
		...
		// Make exchange and attempt to get response as TAXII 1.1 StatusMessage, return
		// feedback to the front-end based on response
		StatusMessage response = taxiiService.getTaxii11RestTemplate().submitInbox(association.getServer(),
				inboxMessage, url);
	...
	public String publish(Taxii21Association association, Map<String, UploadedFile> fileMap) {
		...
		for (Map.Entry<String, UploadedFile> uploadedFile : fileMap.entrySet()) {
			...
			eventService.createEvent(EventType.STATUS_CREATED, String
					.format("Created Status with ID '%s'. Overall status: %s", response.getId(), response.getStatus()),
					association);
		}
	...
	public URI getUploadUrl(Taxii11Association association) {
		// The logic below is a result of the TAXII 1.1 Specification. If the logic is
		// unclear, it is recommended that you review the TAXII 1.1 Specification.
		// Any TAXII 1.1 Collection can have *zero to many* "Receiving Inbox Services",
		// and any TAXII 1.1 Server can have *zero to many* "Service Instances" that can
		// be "INBOX" services
		// Based on this, we first attempt to find *any* address in the Collection's set
		// of services, and if that fails, we attempt to find *any* address in the
		// Server's set of
		// services. This is a lazy approach, and we will not retry if the first address
		// fails.
		Optional<String> url =
				// Assume there will probably only be one "receiving inbox service", find the
				// first one that has an address
				association.getCollection().getCollectionObject().getReceivingInboxServices().stream()
						.filter(inboxService -> inboxService.getAddress() != null // Filter by address is non-null
								&& !inboxService.getAddress().isEmpty() // Filter by address is non-empty
								&& inboxService.getMessageBindings().contains(Constants.HEADER_TAXII11_MESSAGE_BINDING))
						.findFirst() // Filter by "is version 1.1"
						.map(InboxServiceBindingsType::getAddress); // Get address

		// If we cannot determine the Inbox address from the collection itself, try the
		// Server the collection is associated with
		if (!url.isPresent()) {
			if (association.getServer().getServiceInstances() == null
				...
				throw new IllegalStateException(String.format("No inbox services found for collection '%s'",
						association.getCollection().getName()));
			} else {
				...
				url = association.getServer().getServiceInstances(ServiceTypeEnum.INBOX).stream()
						.filter(instance -> instance.getAddress() != null && !instance.getAddress().isEmpty()
								&& instance.getMessageBindings().contains(Constants.HEADER_TAXII11_MESSAGE_BINDING))
						.findFirst().map(ServiceInstanceType::getAddress);
			}
		}
	...
	public URI getUploadUrl(Taxii21Association association) {
		ApiRoot apiRoot = association.getServer().getApiRootObjects().stream()
				.filter(apiRootObj -> apiRootObj.getEndpoint().equals(association.getCollection().getApiRootRef()))
				.findFirst().orElseThrow(IllegalStateException::new);
		return association.getServer().getCollectionObjectsUrl(apiRoot.getEndpoint(),
				association.getCollection().getCollectionObject().getId());
	}
