...
public class CollectionResourceTest {
    ...
    @Test
    public void purgeCollectionTest() {
        ResponseEntity<String> response = collectionResource.purgeCollection(TestData.taxii11Server.getLabel(), TestData.taxii11Collection.getId());
        String collectionResponse = response.getBody();
        assertNotNull(response.getBody());
        assertEquals(200, response.getStatusCodeValue());
        assertEquals("Successfully purged collection '"+ TestData.taxii11Collection.getId()+"'", collectionResponse);
    }
    
    @Test(expected = CollectionNotFoundException.class)
    public void purgeCollectionNoCollectionTest() {
        ResponseEntity<String> response = collectionResource.purgeCollection(TestData.taxii11Server.getLabel(), TestData.taxii11Collection.getId()+"extraId");
    }
    
    @Test(expected = ServerNotFoundException.class)
    public void purgeCollectionNoServerTest() {
        when(serverService.findOneByLabel(TestData.taxii11Server.getLabel())).thenReturn(Optional.empty());
        ResponseEntity<String> response = collectionResource.purgeCollection(TestData.taxii11Server.getLabel(), TestData.taxii11Collection.getId());
    }
