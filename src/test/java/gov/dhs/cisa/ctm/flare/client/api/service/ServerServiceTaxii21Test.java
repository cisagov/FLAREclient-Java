...
import static gov.dhs.cisa.ctm.flare.client.TestData.taxii21Discovery;
import static gov.dhs.cisa.ctm.flare.client.TestData.taxii21Server;
...
public class ServerServiceTaxii21Test {
	...
	public void testCreateTaxii21Server() {
		...
		when(taxiiService.getTaxii21RestTemplate().getCollections(any(), any()))
				.thenReturn(new Collections(Arrays.asList(TestData.taxii21CollectionObject)));
		...
		Optional<Taxii21Server> maybeServer = serverRepository
				.findOneTaxii21ByLabelIgnoreCase(taxii21Server.getLabel());
	...
	public void testCreateTaxii21ServerWithBasicAuth() {
		...
		Optional<Taxii21Server> maybeServer = serverRepository
				.findOneTaxii21ByLabelIgnoreCase(taxii21Server.getLabel());
		assertTrue(maybeServer.isPresent());
		assertFalse(ServerCredentialsUtils.getInstance().getServerCredentialsMap().get("user")
				.get(taxii21Server.getLabel()).isEmpty());
	}
	...
	public void testRefreshTaxii21Server() {
		...
		when(taxiiService.getTaxii21RestTemplate().discovery(any(Taxii21Server.class)))
				.thenReturn(TestData.taxii21Discovery);
		when(taxiiService.getTaxii21RestTemplate().getApiRoot(any(), any())).thenReturn(TestData.taxii21ApiRoot);
		when(taxiiService.getTaxii21RestTemplate().getCollections(any(), any()))
				.thenReturn(new Collections(Arrays.asList(TestData.taxii21CollectionObject)));
